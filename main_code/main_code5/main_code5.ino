...
float heading, headingDegrees,Angle_Goal;
...
switch(phase){
      ...
      case 4:{
        if(phase_state != 4){
            Serial.println("Mode-F: Moved completed");
            sensorsend();
            phase_state = 4;
            LEDsetting(6);
            motor(0,0);
            finaldistance = CalculateDis(GOAL_lng,GOAL_lat,gps_longitude,gps_latitude);
            advanceresult = (startdistance - finaldistance >= 20.0);
      }
